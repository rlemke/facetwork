...
> It has been over a month now. Claude and I have come close to breaking up now and then :-) But I am amazed how far it has come with serious detailed help from me.
> If I do a quick grading there are the areas: 1. A+ : Creating a DSL, truely amazed how it created the AST tree and serialization and parsing. 2. A : Create focus api to other services (ie MongoDb, PostGres, downloads, etc) 3: B+ Understaning the overall intent, that is, this is a workflow language, 4: C: implementing the workflow engine in a distributed fashsion. It kept try to make it operate like a controlling process like Jenkins. This is where we had some serious layout of plans and examples. Now I think it understand.
...
> So if I have not learned Python yet, I suspect many others are in the same boat. I am worried about that for key validations.
...
> The good news is that this has made programing fun again. I get to develop my ideas without speeding days if not weeks on learning api and interface behavior that may or may not be correct.
