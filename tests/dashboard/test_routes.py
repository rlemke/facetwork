...
class TestLockRoutes:
    ...
    def test_lock_list_empty(self, client):
        tc, store = client
        resp = tc.get("/locks")
        assert resp.status_code == 200
        assert "Locks" in resp.text

    def test_lock_list_with_data(self, client):
        tc, store = client
        self._make_lock(store, "step:wf-1:s-1", topic="osm.*", handler="osm.CacheLookup")
        resp = tc.get("/locks")
        assert resp.status_code == 200
        assert "step:wf-1:s-1" in resp.text

    def test_lock_detail(self, client):
        tc, store = client
        self._make_lock(store, "step:wf-1:s-1", topic="osm.*")
        resp = tc.get("/locks/step:wf-1:s-1")
        assert resp.status_code == 200
        assert "step:wf-1:s-1" in resp.text

    def test_lock_detail_not_found(self, client):
        tc, store = client
        resp = tc.get("/locks/nonexistent")
        assert resp.status_code == 200
        assert "not found" in resp.text.lower()

    def test_lock_detail_shows_meta(self, client):
        tc, store = client
        self._make_lock(store, "step:wf-1:s-1", topic="osm.*", handler="osm.CacheLookup")
        resp = tc.get("/locks/step:wf-1:s-1")
        assert resp.status_code == 200
    ...
    def test_release_lock(self, client):
        tc, store = client
        self._make_lock(store, "step:wf-1:s-1")
        resp = tc.post("/locks/step:wf-1:s-1/release", follow_redirects=False)
        assert resp.status_code == 303
        assert store.check_lock("step:wf-1:s-1") is None

    def test_api_locks_empty(self, client):
        tc, store = client
        resp = tc.get("/api/locks")
        assert resp.status_code == 200
        assert resp.json() == []

    def test_api_locks_with_data(self, client):
        tc, store = client
        self._make_lock(store, "step:wf-1:s-1", topic="osm.*")
        resp = tc.get("/api/locks")
        assert resp.status_code == 200
