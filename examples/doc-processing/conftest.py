"""Root conftest for doc-processing example.
...
def _ensure_doc_handlers():
    """Purge non-doc-processing handler modules from sys.modules."""
    for key in list(sys.modules.keys()):
        if key == "handlers" or key.startswith("handlers."):
            mod = sys.modules[key]
            mod_file = getattr(mod, "__file__", "") or ""
            if "doc-processing" not in mod_file:
                del sys.modules[key]
