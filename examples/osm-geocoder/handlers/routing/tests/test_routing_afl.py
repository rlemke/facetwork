"""Tests that routing adapter AFL files compile without errors."""
...
# All routing AFL files that should compile
AFL_FILES = sorted(AFL_DIR.glob("*.afl"))

# Dependency files needed for use statements
DEPS_DIR = Path(__file__).resolve().parent.parent.parent
TYPES_AFL = DEPS_DIR / "cache" / "afl" / "osmtypes.afl"
VIZ_AFL = list((DEPS_DIR / "visualization").rglob("*.afl"))
...
def _compile_with_deps(*afl_paths: Path):
    """Compile AFL files with their dependencies. Returns a Program AST."""
    sources = []
    ...
    types_file = AFL_DIR / "routing_types.afl"
    if types_file.exists():
        sources.append(_read(types_file))
    for p in afl_paths:
        if p.name != "routing_types.afl":
...
def test_api_adapter_defines_facets():
    ...
    program = _compile_with_deps(AFL_DIR / "routing_api.afl")
...
def test_osrm_adapter_defines_facets():
    ...
    program = _compile_with_deps(AFL_DIR / "routing_osrm.afl")
    facet_names = _all_event_facet_names(program)
    assert "Route" in facet_names
    assert "MultiStopRoute" in facet_names
    assert "Isochrone" in facet_names
