...
def _restaurant_feature(
    lon: float, lat: float, name: str = "Test", amenity: str = "restaurant", cuisine: str = ""
) -> dict[str, Any]:
...
class TestDemographicsExtractor:
    ...
    def test_population_density(self, tmp_path):
        ...
        tiger_path = _make_tiger_geojson(
            [
                _county_feature("01001", "Test County", "01", aland=2_000_000_000),  # 2000 km²
            ]
        )
        acs_path = _make_acs_csv(
            [
...
class TestScoring:
    ...
    def scored_data(self, tmp_path):
        ...
        demo_features = [
            _county_feature(
                ...
                geometry={
                    "type": "Polygon",
                    "coordinates": [
                        [[-86.5, 32.0], [-85.5, 32.0], [-85.5, 33.0], [-86.5, 33.0], [-86.5, 32.0]]
                    ],
                },
            ),
            _county_feature(
                ...
                geometry={
                    "type": "Polygon",
                    "coordinates": [
                        [[-88.0, 34.0], [-87.0, 34.0], [-87.0, 35.0], [-88.0, 35.0], [-88.0, 34.0]]
                    ],
                },
            ),
        ]
        ...
        restaurants = []
        for i in range(50):
            restaurants.append(_restaurant_feature(-86.0 + i * 0.01, 32.5, f"Urban Rest {i}"))
        restaurants.append(_restaurant_feature(-87.5, 34.5, "Rural Rest 1"))
        restaurants.append(_restaurant_feature(-87.3, 34.3, "Rural Rest 2"))
    ...
    def test_point_in_polygon_assignment(self, scored_data):
        ...
        counts = _count_restaurants_per_county(demo["features"], rest["features"])
    ...
    def test_restaurants_per_1000(self, scored_data, tmp_path):
        ...
        urban = [f for f in scored["features"] if f["properties"]["NAME"] == "Urban County"][0]
    ...
    def test_zero_population_county(self, tmp_path):
        ...
        demo_features = [
            _county_feature("01099", "Empty County", "01", aland=1_000_000_000),
        ]
        demo_features[0]["properties"].update(
            {
                "population": 0.0,
    ...
    def test_empty_restaurants(self, tmp_path):
        ...
        assert props["suitability_score"] == round(props["demand_index"] * 100.0, 2)
