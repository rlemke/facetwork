...
Since that answer was written the library has grown to twenty-one facets in four namespaces, and I should report what growing it taught me, because two of the three lessons are unflattering.

The selection rule held, and it is narrow: a facet earns its place when a survey finds the same non-domain logic rewritten across many domains *and getting it wrong differently each time*. `fw.http` qualified — fourteen of twenty-three domains hand-rolled download-plus-cache, one frozen forever on a moving `…-latest` target, one recording a cache key it never compared. `fw.archive` qualified — nine domains, seventeen sites. Both are cases where §13.3's argument had been quietly lost inside my own system, which is the strongest evidence I have that "leave it to the handler author" is a real cost rather than a rhetorical one.

The first unflattering lesson is that adding ambient declarations exposed two latent defects that no test covered: `use fw.file` was rejected as an unknown namespace, and the runtime resolved facets only against a workflow's stored AST — which, by construction, does not contain facets that ship with the framework. The first had been failing a test since the day `fw.file` landed and I had not noticed; the second only appeared when a facet was exercised on a real runner rather than in unit tests. Both were consequences of the design decision, not accidents of implementation, and I did not anticipate either.

The second is about `fw.exec`, which invokes an external command and is the direct analogue of Snakemake's `shell:`. I built it and I want to be candid that it fails the selection rule above: all forty-eight `subprocess` sites in the domain packages sit inside Python handlers, where `subprocess` is already available, so it removes no duplication whatever. Its justification is reach for an author who writes no Python — which is a real justification, but a weaker one than the other three namespaces have, and it deserves to be labelled as such rather than presented as more of the same.

It also forced an asymmetry with `shell:` that I would rather not need. `Run` takes an argv list and never invokes a shell, which is a plain improvement — Snakemake interpolates wildcards into a shell string, and a filename containing a space or `$(…)` changes what runs. But `Run` also requires the command to be named in the runner's `FW_EXEC_ALLOW`, and unset means refuse. Snakemake needs no such gate because the person writing the Snakefile owns the machine. Facetwork had already assumed otherwise — the `script { }` sandbox removes the `open` builtin and allowlists imports precisely because the FFL author is not fully trusted — and an ungated exec would have erased that in a single facet. The cost is that a workflow which would run anywhere under Snakemake needs a provisioned host here, and I do not think "it works on the host that was set up for it" is as good a property as "it works". I take the trade because the alternative is a security boundary that exists in one part of the language and not in another, which is worse than either consistent choice.
