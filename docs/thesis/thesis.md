...
- **Part III — Comparison and Evaluation**
  ...
  - Chapter 13. Nextflow, Snakemake, and the Scientific Pipeline Tradition
  - Chapter 14. Evaluation: The OSM Geocoder
- **Part IV — Closure**
  - Chapter 15. Limitations and Open Problems
...
### 13.1 Why this chapter is the hardest one

Chapters 9-12 compare Facetwork against systems whose designs differ from it fundamentally: Temporal's determinism contract, Camunda's graphical standard, Airflow's central scheduler, Jenkins's single controller. This chapter is different, and more uncomfortable, because Nextflow and Snakemake are the *closest* relatives in the landscape. They are declarative workflow DSLs, they infer dependency graphs from typed inputs and outputs, they run heterogeneous work across whatever compute is available, and they are used overwhelmingly for exactly the shape of work this thesis's own evaluation uses — bulk geospatial and genomic pipelines that fan out over a collection and merge. Jenkins gets a chapter and is explicitly not a competitor. Nextflow genuinely is one, and it has tens of thousands of users to Facetwork's zero.

The comparison below is therefore measured rather than argued. Snakemake 9.25.1 and Nextflow 26.04.6 were installed on the same host that runs this thesis's fleet, and the same pipeline — a twelve-way fan-out of three-second tasks followed by a merge — was written three times and run.

### 13.2 What the two systems are

**Snakemake** [KR12] descends from `make`. A rule declares output patterns, input patterns and a shell body; the engine works backwards from a requested target, matching wildcards against file paths to infer which rules must run. Dependency inference is *filesystem-shaped*: a file's existence and modification time answer the question "is this step done?".

**Nextflow** [DTP+17] descends from dataflow programming. Processes are connected by channels; a process runs once per item arriving on its input channel, in an isolated work directory. Completion is recorded in a session cache keyed by a hash of the task's inputs, code and container, and `-resume` consults that cache.

Both target the same operational reality Facetwork does — long jobs, heterogeneous binaries, a cluster you do not own — and both solve it by submitting to a scheduler (SLURM, Kubernetes, a cloud batch service) rather than by having workers claim work.

### 13.3 Measurements

| | cold run | re-run, nothing changed | resume after interruption |
|---|---|---|---|
...
**The same argument, in someone else's codebase.** Every measurement above is on a workflow I wrote, which is the right way to isolate an orchestrator and also a way to flatter one. So the retrieve stage of [PyPSA's `grid-builder`](https://github.com/PyPSA/grid-builder) — a Snakemake workflow in the PyPSA organisation, not written for this thesis — was ported to FFL and both were run against the same OpenStreetMap extract. They produce the **same data**: identical columns, 832 identical ids, zero rows differing (the full comparison, and the byte-level caveat about `earth_osm`'s unstable tag ordering, are in [`experiments/grid-builder`](experiments/grid-builder/)).

What matters here is what its retrieve rule declares: **no file inputs**. Nothing can therefore make its outputs stale, so after the first successful run the rule is permanently up to date and re-running the workflow a year later serves year-old OSM data, reporting success. That is not a hypothetical constructed to make the point — it is the shape the make model produces when a rule's real input is a remote resource rather than a file, and it is why `force_redownload` exists in its config, to be set by a human who already suspected. The FFL port re-derives unprompted, because the extractor's version no longer matches the one recorded beside the artifact.

The port also cost three defects that only appeared on a fleet — a storage claim that was true on a laptop and false against an object store, a fan-out racing on a shared cache file, and a fork deadlock inside a multi-threaded runner. They are catalogued in the experiment because they are the honest other side of this section: the reuse guarantee argued for here is claimed in the same setting that produces those failures, and a comparison that reports only the favourable half is not a comparison.
...
### 13.4 The completion signal, and a demonstration

The most consequential difference is not performance but what each system trusts as evidence that a step finished.

Snakemake trusts the output file. To see what that means, `out/05.txt` was truncated to zero bytes — simulating a write interrupted by a full disk, an OS kill, or a network filesystem glitch — and the pipeline re-run. Snakemake reported nothing to do, and the file remained empty. The shard was not re-executed, because the file exists and is newer than its inputs. This is not a bug; it is `make` semantics working as designed, and Snakemake provides `--rerun-incomplete` plus its own incomplete-file metadata for the cases it knows about. But damage that happens *outside* the engine's knowledge is invisible to it, and the pipeline will happily build a merged output from a truncated shard.
...
|---|---|---|---|
| **Topology description** | rules with wildcard file patterns; graph inferred backwards from a target | processes wired by channels; graph is the dataflow | facets and `andThen` blocks; graph inferred from references, with `after` for invisible edges |
| **Coordination** | one process expands the DAG and submits | one process expands the DAG and submits | leaderless; runners claim atomically, no submitter |
| **Recovery** | outputs on disk; timestamp comparison | session cache of task hashes; `-resume` | persisted step state machine; recovery is re-reading state |
| **Updatability** | edit the Snakefile, re-run | edit the script, re-run | handlers hot-reload from the registry; a running workflow picks up new handler code |
| **Observability** | logs and the DAG render | an execution report and trace file, well-developed | a live execution graph, step logs, and a repair surface |
| **Operational cost** | very low — a Python package | low — a JVM launcher | high — MongoDB, an object store, and runners |
| **Domain accessibility** | high for anyone comfortable with `make` and shell | high in bioinformatics, where it is the lingua franca | unproven; the DSL is smaller but its community is zero |

The row that should be read most carefully is operational cost. Snakemake is `pip install snakemake` and a text file. Facetwork is a database, an object store, a fleet, and this thesis. For the single-machine bulk pipeline that is the scientific tradition's bread and butter, that difference is not a rounding error — it is the whole decision.

### 13.6 Where Facetwork is genuinely different

Three properties survive the comparison, and only three.

**Live updatability.** A Snakemake or Nextflow run executes the workflow definition it started with; changing the pipeline means stopping and re-running. Facetwork's handlers are registered in a database and reloaded, so a handler fix reaches an in-flight workflow (§6). For a run measured in hours across thousands of shards — the county atlas of §14 — that is the difference between fixing a bug and restarting a day's work.

**Leaderless execution.** Both comparators have a submitting process that owns the run. Facetwork's coordination is an atomic claim on a document, so runners are disposable and the machine that submitted the workflow is not special. This buys the informal-fleet model this thesis argues for, and it costs the per-step overhead measured above.

**Step-level recovery independent of artifacts.** Recovery does not depend on what is on disk, which is what makes `Re-run From Here`, the repair surface, and mid-run reclaim possible.

Everything else on the comparison — dependency inference, typed step interfaces, heterogeneous execution, fan-out — the scientific tradition already had, and in several respects had better. The defense's concessions stand: CWL's file-object type system is more developed than FFL's, Snakemake's wildcard pattern-matching has no FFL analogue, and Nextflow's runtime workflow expansion is a capability gap. This chapter adds one more: incremental recomputation, which both comparators provide as a runtime guarantee and Facetwork leaves to handler authors.

### 13.7 The honest recommendation

If the work is a bulk pipeline on machines you control, and the failure mode you fear is "a step crashed and I want to resume", Snakemake or Nextflow is the better choice and the cheaper one. If the workflow must survive the loss of the machine that launched it, must be updatable while running, or must spread across borrowed hardware that comes and goes, the comparison changes — and those are the conditions under which the rest of this thesis is written.
...
- Single-block fan-out — one `andThen foreach` — of up to 255 concurrent branches. Reaching this width required the continuation-coalescing fix of §5.7.5 and §15.5; below roughly a few dozen branches the un-coalesced evaluator behaved acceptably, and the O(N²) edge only became load-bearing at the hundreds-of-branches scale.
...
The cross-package demonstration of §15.2 — porting `research-agent` to consume `anthropic.messages.CreateMessage` from `fwh_anthropic` — surfaced three latent runtime issues that the original OSM-only evaluation had not exercised. All three were corrected; they are worth recording briefly both because they illustrate the kinds of bug the state-recovery model makes visible and because the fixes themselves form part of the current codebase.
...
Each of these three issues has a regression test in the runtime test suite, exercising the patterns that surfaced them (statement-level fan-out, foreach iteration, inline-andThen back-references). Together they advance the §14.5 evaluation claim — that the design survives crashes, hiccups, and handler bugs without losing more than one stage's work — by an additional point: the state-recovery model also makes *internal* runtime bugs diagnosable through the dashboard rather than through gdb.

### 14.5 Continuation coalescing under data-bounded fan-out (note added in proof)

A fourth note-in-proof issue surfaced from a workload the Chapter 14 evaluation had not previously run at *width*: converting every cached region's PBF to GeoJSON in a single `andThen foreach` over the cache listing — 255 parallel branches, the widest single fan-out the fleet had executed. Every hand-written workflow to that point fanned out to at most a few dozen branches, and the block-mediated advancement of §5.8 had carried them without incident. At 255 the actor model's *correctness* held but its *efficiency* did not: the runners filled with `resume_step` passes that did no productive work (logging `iterations=0`), the foreach's leaf conversions stopped being claimed, and throughput collapsed after the first few dozen completions.
