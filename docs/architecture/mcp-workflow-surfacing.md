...
The `osm.geocode` namespace (`https://github.com/rlemke/fwh_osm/blob/main/src/osm_geocoder/ffl/geocoder.ffl`) has one schema (`GeoCoordinate`), one event facet (`Geocode`), and two workflows (`GeocodeAddress`, `GeocodeAll`). What does surfacing it as a dedicated per-namespace MCP server look like end-to-end?
