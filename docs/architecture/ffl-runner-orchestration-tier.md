...
**Status:** Steps 1–3 of the rollout (§7) are **implemented**. Step 1 = the `--continuation` mode flag; step 2 = the `runtime_version` gate (both no-ops by default); step 3 = the `runner-ffl` compose service (orchestration tier, `continuation_mode=shared`, core-engine-only), brought up ≥2 and validated. Remaining: wire `runner-ffl` into the central `fleet_config`/`fleet-agent` role loop and into `start-all-workers` (so it comes up automatically fleet-wide), then step 4 (flip handler runners to `--continuation off`) and step 5 (observe/tune).
...
3. ✅ **Done (compose)** — `runner-ffl` service in `docker-compose.full-stack.yml`: same core image, entrypoint overridden to run the runner directly with `AFL_CONTINUATION_MODE=shared`, **no example handlers / no `--registry`** (pure orchestration — only the `fw:execute` builtin + the sweep, so heavy handler work can't starve it), `server_group=ffl-runner`. Bring up ≥2 with `start-runner --fleet --example ffl --replicas 2` (managed by `rebuild-workers` too). **Remaining:** add an `ffl-runner` role to the central `fleet_config` + `fleet-agent` apply loop (currently only `osm-geocoder`/`gh-router` are central roles) and to `start-all-workers`, so the tier comes up automatically on every host rather than via a manual bring-up.
