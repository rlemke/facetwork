...
**Status:** Steps 1–3 of the rollout (§7) are **implemented**. Step 1 = the `--continuation` mode flag; step 2 = the `runtime_version` gate (both no-ops by default); step 3 = the `runner-ffl` compose service + the central `ffl-runner` role wiring (`fleet set --ffl-replicas N` / `--ffl-image`, brought up per host by `fleet-agent`). The role is **off by default** (`replicas=0`), so existing fleets are unaffected. Remaining: `start-all-workers` integration, then step 4 (flip handler runners to `--continuation off`) and step 5 (observe/tune). NB: the runner image rolled to the fleet must include steps 1–3 for `shared` mode to take effect — activate the tier with `fleet set --ffl-replicas N` *after* such an image roll.
