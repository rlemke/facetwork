...
3. ✅ **Done** — `runner-ffl` service in `docker-compose.full-stack.yml` (same core image, entrypoint overridden to run the runner directly with `AFL_CONTINUATION_MODE=shared`, **no example handlers / no `--registry`** — pure orchestration: only the `fw:execute` builtin + the sweep, so heavy handler work can't starve it; `server_group=ffl-runner`). Fleet-join override in `docker-compose.fleet.yml` (image defaults to `AFL_RUNNER_IMAGE`, pinnable via `AFL_FFL_IMAGE`). Central role wiring: `scripts/fleet set --ffl-replicas N [--ffl-image TAG]` records an `ffl-runner` role; `scripts/fleet-agent` brings up `runner-ffl` at that replica count per host (non-fatal, like `gh-router`). Off by default (`replicas=0`). Also managed by `rebuild-workers` and bring-up-able directly via `start-runner --fleet --example ffl --replicas N`. **Remaining:** add it to `start-all-workers` so the local full-stack brings it up automatically too.
4. ✅ **Done** — handler-runner mode is env-gated (`AFL_CONTINUATION_MODE: ${AFL_HANDLER_CONTINUATION_MODE:-inline}` on the `*s3-storage`/`*osm-s3-env` anchors), default `inline` so a bare `docker compose up` self-orchestrates. `start-all-workers` brings up the ffl tier first, then sets `AFL_HANDLER_CONTINUATION_MODE=off` for the handler runners. Validated live: with handlers `off` + 2 ffl-runners `shared`, a workflow completed promptly (inline cascade advances handler steps; the ffl tier owns the sweep).
