...
**Status:** Steps 1–4 of the rollout (§7) are **implemented**; step 5 (observe/tune) is operational. Step 1 = the `--continuation` mode flag; step 2 = the `runtime_version` gate (both no-ops by default); step 3 = the `runner-ffl` compose service + central `ffl-runner` role wiring (`fleet set --ffl-replicas N` / `--ffl-image`, brought up per host by `fleet-agent`; off by default at `replicas=0`); step 4 = env-gated handler-runner mode (`FW_HANDLER_CONTINUATION_MODE`, default `inline`) flipped to `off` by `start-all-workers` after the tier is up. A bare `docker compose up` stays self-orchestrating (`inline`), so existing fleets are unaffected until explicitly opted in. NB: to activate on the remote fleet, roll a runner image that includes steps 1–4 (`buildx → fleet set --image`) and `fleet set --ffl-replicas N` *before*/with flipping handlers — `shared`/`off` are inert on an older image.
...
1. ✅ **Done** — `--continuation {inline,off,shared}` mode flag (`FW_CONTINUATION_MODE`), default `inline` = today's behaviour. Gates the `_fw_continue` poll (`RegistryRunner`) and the stuck-step sweep (both runner classes) via `BaseRunnerConfig.polls_shared_continuations()` / `runs_stuck_step_sweep()`.
...
3. ✅ **Done** — `runner-ffl` service in `docker-compose.full-stack.yml` (same core image, entrypoint overridden to run the runner directly with `FW_CONTINUATION_MODE=shared`, **no example handlers / no `--registry`** — pure orchestration: only the `fw:execute` builtin + the sweep, so heavy handler work can't starve it; `server_group=ffl-runner`). Fleet-join override in `docker-compose.fleet.yml` (image defaults to `FW_RUNNER_IMAGE`, pinnable via `FW_FFL_IMAGE`). Central role wiring: `fw fleet set --ffl-replicas N [--ffl-image TAG]` records an `ffl-runner` role; `fw fleet agent` brings up `runner-ffl` at that replica count per host (non-fatal, like `gh-router`). Off by default (`replicas=0`). Also managed by `rebuild-workers` and bring-up-able directly via `start-runner --fleet --example ffl --replicas N`. **Remaining:** add it to `start-all-workers` so the local full-stack brings it up automatically too.
4. ✅ **Done** — handler-runner mode is env-gated (`FW_CONTINUATION_MODE: ${FW_HANDLER_CONTINUATION_MODE:-inline}` on the `*s3-storage`/`*osm-s3-env` anchors), default `inline` so a bare `docker compose up` self-orchestrates. `start-all-workers` brings up the ffl tier first, then sets `FW_HANDLER_CONTINUATION_MODE=off` for the handler runners. Validated live: with handlers `off` + 2 ffl-runners `shared`, a workflow completed promptly (inline cascade advances handler steps; the ffl tier owns the sweep).
