...
**Dependency analyzer + test-suite hardening.** Added `fw install check [--install]` — statically scans `tests/` + `examples/` for imported modules (hard imports AND `pytest.importorskip`), reports what's not importable on this host (excluding local path-hacked example modules), and lists example packages vs installed `facetwork.examples` entry points; `--install` fixes the gaps (example modules via `fw install example`, boto3 via the `[s3]` extra, else pip; `--user`/PEP 668-aware). Brought all 4 fleet hosts to an identical, fully-passing suite (3247 passed) — incl. giving server1 a repo `.venv` (`fw install repo`) since its system Homebrew python is PEP 668-locked. Three pre-existing test issues fixed along the way: `test_config` now strips ambient `AFL_*` env (hermetic); a root `examples/conftest.py` rebinds each example's `handlers` package per-test (they collided in `sys.modules`); and the dashboard's s3 `artifact_url` no longer doubles the `/output/` segment.

**Fixes:** `fw maint purge-servers` mis-parsed `--mongo` (its arg loop used `for a in "$@"` + `shift`, which doesn't advance the loop, so `--mongo URL` tried to connect to a host literally named `--mongo`); switched to a `while [ $# -gt 0 ]` / `shift 2` loop like the other `fw` commands. Capped `fastapi>=0.100,<0.136` in the `dashboard` extra — it was unbounded, so a fresh install pulled 0.138 whose lazy `include_router` makes `app.routes` hold opaque `_IncludedRouter` entries with no `.path`, breaking the route-introspection tests; the fleet + dashboard image run 0.135.x.
...
Fixed a runtime **livelock on large `foreach` fan-outs** (e.g. converting all 255 cached OSM regions, each running an event facet) and added the facet that surfaced it. Symptom: runners spun on `resume_step` (logging `resume_step done iterations=0` hundreds of times/sec) and stopped claiming the pending event tasks, so only a handful of sub-blocks ran before everything stalled. Two compounding causes, both fixed.

**Continuation coalescing (`evaluator.py` / `persistence.py` / `registry_runner.py` / `mongo_store/tasks.py` / `memory_store.py`):**
- **Generation-time dedup**: when a foreach child progresses it re-dirties the parent block, and previously each of the N children enqueued *another* continuation for that same block (random uuid, marked COMPLETED not deleted after processing) — an O(N²) "continuation storm" that flooded the runner thread pool and starved handler execution. `process_single_step()` now filters `remaining_dirty` against `Persistence.get_pending_continuation_step_ids(workflow_id)` (default scan, overridable), so at most one PENDING continuation per block is enqueued. Checks PENDING state only — a child event arriving *after* a continuation is claimed still generates a fresh one (no lost events).
- **Claim-time coalescing**: `_process_continuation()` calls `Persistence.delete_pending_continuations_for_step(step_id, except_task_id=task.uuid)` (mongo `delete_many`, memory filter, base no-op) at the start of processing — that one re-evaluation already reflects all children's current state, so the other queued siblings are redundant and are deleted rather than each claimed and processed to a no-op. This is the design's intended coalescing and closes the generation-time race (two enqueued concurrently → whichever is claimed first collapses the rest). Continuations enqueued *after* processing begins reflect newer child events and are untouched.
...
**Run safety**: handler **preflight** — `run` refuses with `CatalogRunBlocked` if any event facet reachable from the entry workflow has no loadable handler in the fleet registry (catches declared-but-unimplemented facets before launch, instead of dead-lettering mid-flight). `RegistryDispatcher.check_loadable` imports each handler module + resolves its entrypoint (catching an unimplemented or unimportable handler before launch). A broken *lazy* import inside a dispatched handler body surfaces at dispatch, not preflight — handler modules host many facets' handlers behind one entrypoint, so a module-level scan can't attribute a sibling's broken import to this facet without false-positives. **Execution isolation** — each run mints a fresh execution `workflow_id` + per-run `WorkflowDefinition` (was reusing the revision's definition id, so a prior terminated run's state collided with the next).
...
- **Handler resources**: two new MCP resources — `fw://handlers` (list all registrations) and `fw://handlers/{facet_name}` (detail by facet name); routed in `_handle_resource()` with not-found error handling
