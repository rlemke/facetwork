...
> stable **name** instead: set `FW_INFRA_IP=<infra host IP>` in `.env.fleet` and use
...
- **The Docker daemon must trust the plain-HTTP registry** at
  ...
  registry, i.e. the `FW_INFRA_IP` you set in `.env.fleet`. Substitute your
  fleet's value (e.g. find it with `grep FW_INFRA_IP .env.fleet`). Without this
...
#    REQUIRED in .env.fleet.override: set FW_DATA_DIR to a big LOCAL path on THIS
#    server (it ships empty — do not skip). macOS: e.g. $HOME/afl_data ; Linux:
#    e.g. /var/lib/afl_data. Create it first — `mkdir -p "$FW_DATA_DIR"`. Do NOT
...
#    FW_OSM_REPLICAS / FW_INFRA_IP there too. start-runner loads .env.fleet then
...
fw runner start --fleet                  # FW_OSM_REPLICAS from .env.fleet(.override)
...
its `FW_MONGODB_URL`/name still point at the *source* host, so its containers
...
```bash
git pull
fw fleet env-migrate            # moves FW_DATA_DIR / FW_OSM_REPLICAS /
                                     # FW_RUNNER_NAME(/FLEET_HOST) into
...
> **Pass `--data-dir` (don't rely on `export FW_DATA_DIR`).** The agent needs a
...
> `FW_DATA_DIR` is fragile: it isn't inherited by an already-running `watch`
...
- **Secrets (prod).** For real deployments, don't keep MinIO creds in each
  ...
  `FW_FLEET_KEY` out-of-band), `fw fleet secret set --minio-access … --minio-secret …`.
  Each server then needs only `FW_FLEET_KEY` (one rotatable secret); `fleet-agent`
...
- **Endpoint reachability.** The MinIO/Mongo addresses must resolve from both the
  ...
  **DNS A-record**, never `localhost` and never a `.local`/mDNS name (containers
  can't resolve those). For a container-resolvable alias, set `FW_INFRA_IP` and
