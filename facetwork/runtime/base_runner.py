...
def measured_resources() -> dict:
    """Measured capacity of this host, cached for the process.

    Module-level, NOT a BaseRunner method: AgentPoller is a plain class and does
    not inherit from BaseRunner, so a method here would be unavailable to one of
    the three runner implementations — which is exactly how the first cut of
    this shipped inert.

    Capacity is treated as static for the process lifetime; a runner that wants
    to react to a filling disk should restart.
    """
    global _RESOURCES_CACHE
    if _RESOURCES_CACHE is None:
        _RESOURCES_CACHE = _measure()
