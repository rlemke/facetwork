...
_GRAMMAR_PATH = Path(__file__).parent / "grammar" / "ffl.lark"
...
class FFLParser:
    ...
    The Lark instance is shared across all FFLParser instances since the
    ...
    def parse(
        ...
        """Parse FFL source code and return an AST.

        Args:
            source: FFL source code string
        ...
        transformer = FFLTransformer(source_id=effective_source_id)
    ...
    def parse_file(self, filepath: str | Path) -> Program:
        """Parse an FFL file and return an AST.

        Args:
            filepath: Path to the FFL source file
