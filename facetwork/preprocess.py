...
def preprocess_script_braces(source: str) -> str:
    ...
    Args:
        source: FFL source code (may contain brace-delimited scripts).
    ...
    while i < length:
        ...
        if _match_keyword(source, i, "script"):
            ...
            # skip whitespace (not newlines — FFL rule)
...
def _escape_for_string(code: str) -> str:
    """Escape code content for embedding in a double-quoted FFL string literal."""
    code = code.replace("\\", "\\\\")
    code = code.replace('"', '\\"')
    code = code.replace("\n", "\\n")
    code = code.replace("\t", "\\t")
