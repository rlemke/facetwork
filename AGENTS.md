...
- **`fw util memory-sync [-m MSG] [--dry]`** — commit + push this project's Codex
  memory directory (`~/.Codex/projects/<slug>/memory`, where `<slug>` is the repo
  ...
  (private GitHub repo `rlemke/Codex-memory-facetwork`). No-op when nothing changed,
  ...
  automatic. Only does real work on the host where Codex runs (it errors cleanly on
...
The osm-geocoder example demonstrates a **source adapter pattern** that decouples data extraction (PBF / PostGIS / GeoJSON inputs) from analysis (statistics, filtering, rendering). The detailed contract — namespace layout, schema unification, and composed workflows — lives in the [osm-geocoder repo's AGENTS.md](https://github.com/rlemke/fwh_osm/blob/main/AGENTS.md).
...
Full per-domain descriptions live in `domains.json` and each repo's README/AGENTS.md.
...
- **Codex** can author both FFL definitions and handler implementations from requirements descriptions.
...
- **ClaudeAgentRunner**: LLM-driven in-process execution via Codex API
...
Set `ANTHROPIC_API_KEY` to enable live Codex API calls for prompt-block event facets. Set `FW_POSTGIS_URL` (e.g. `postgresql://afl:afl@afl-postgres:5432/afl_gis`) for PostGIS imports — without it the importer falls back to a hardcoded default that may not match your setup.
...
The MCP server (`python -m facetwork.mcp`) exposes FFL compiler tools, runtime management, and a PostGIS query tool. Configure it in `.mcp.json` for Codex integration.
...
Moved to the `postgis-import` skill (`.Codex/skills/postgis-import/SKILL.md`) — vacuum/tune/kill-vacuum and the disposable local import instance (`FW_IMPORT_POSTGIS_URL`).
...
### How Codex should build and review
...
### How Codex should review changes
