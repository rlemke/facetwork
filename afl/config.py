...
def _apply_env_overrides(config: AFLConfig) -> AFLConfig:
    """Apply environment variable overrides to an existing config.

    Only overrides fields for which the corresponding env var is set.
    Returns a new :class:`AFLConfig` (does not mutate *config*).
    """
...
def load_config(path: str | Path | None = None) -> AFLConfig:
    ...
    if env_name:
        overlay_filename = f"afl.config.{env_name}.json"
    ...
    if base_data:
        config = AFLConfig.from_dict(base_data)
    else:
        config = AFLConfig()
...
_config_cache: AFLConfig | None = None


def get_config() -> AFLConfig:
    """Return the global AFL configuration singleton.
