...
- **Yielding from inside a `foreach` — wrap the value in a list to AGGREGATE.** `yield F(xs = item)` *assigns*, so each iteration overwrites the last and the fan-out's result is the **final** iteration's value; `yield F(xs = [item])` *concatenates* across iterations. Both compile, every iteration's step still runs, and the run reports success either way — so the bare form silently under-reports a fan-out (measured: 3 files → 1 digest vs 3). When the per-iteration value is already a list, yield it unwrapped and it extends. For a real fan-in over artifacts (rather than returned values), order a later step with `after` and have it read the store.
