...
| **Execution semantics — what is guaranteed** — delivery is **at-least-once**, there is **no fencing token**, and Mongo (control) / object store (data) have **no transaction between them**, so handler idempotency is a REQUIREMENT. ⚠️ The real duplicate-execution window is the dead-server reaper, not lease expiry: a runner that is alive but partitioned/paused stops pinging while its handler keeps working, so a handler must be safe to run twice **concurrently**. Effect classes, per-class idempotency rules, and what to measure before trusting it in production | [docs/reference/execution-semantics.md](docs/reference/execution-semantics.md) |
...
| **Built-in `fw.compare` facets** — `Tabular` / `Summarise`, for "did this pipeline reproduce that published result?". Reports the **highest level at which two outputs agree** (exists → schema → cardinality → keys → values → bytes) rather than a boolean. The level names what DID match, locating the first thing that did not — **`keys` is the most informative rung**: the same records exist on both sides and their VALUES differ, so nothing broke in transfer and the disagreement is about METHOD (a verdict of `cardinality` or lower usually means something broke before the science started). ⚠️ Key columns define what "the same record" means and are a substantive claim: without them records match BY FILE POSITION, so one inserted row misaligns everything after it. ⚠️ **`abs_tolerance` for interval scales** (°F/°C, dates, elevations) — a *relative* tolerance is meaningless where zero is arbitrary: a constant −0.9 °F error fails in January (2% of 25.2 = 0.50) and passes in July (2% of 77.5 = 1.55), because a checksum is the wrong predicate: measured on `fwh_gridbuilder`, two orchestrators produced byte-different files containing identical data (unstable nested key order upstream). ⚠️ Normalisations are **declared and reported** in `normalised_by` — silently sorting until two files match manufactures a reproduction rather than demonstrating one. A missing record does not hide the rest: with `key_columns` the intersection is characterised too ("13,978 identical, 1 missing"). `Summarise` reports the **weakest** pair, and an empty verdict list is **not** success. ⚠️ **`out_of_scope` separates scope from content** — unmatched records lying in a key partition (first key column) the other artifact does not cover **at all**. Measured on GIAB HG001: of 242,958 records the ladder called "retired", **99,980 (41.2%) are chrX**, which the newer release simply does not include (`CHROM1-X` vs `1_22`). Reporting content differences between artifacts spanning **different key space** is the ladder's own failure mode one level up, and it runs green. `split_columns` explodes a **delimited multi-value column** into one row per element (a VCF `ALT` of `A,ATTTACAACC` is two alleles sharing a row) — it CHANGES the unit of the counts, so it says so in `normalised_by` with before/after | [examples/canonical/18-builtin-compare-facets.ffl](examples/canonical/18-builtin-compare-facets.ffl) |
| **Built-in `fw.provenance` facets** — `Manifest` / `Verify`: the RUN-level answer to "what produced this, from what, with which software". `fw.http` already writes a per-artifact `<dest>.meta.json`; this records the workflow, step, software (facetwork + every installed `fwh_*`), container, input provenance, and per-output SHA-256. ⚠️ An input with **no recorded provenance is REPORTED**, never omitted — it comes back in `incomplete` and `complete` is false, because a manifest listing only the inputs it understood looks complete while being least trustworthy exactly where it matters. ⚠️ `run_mode` is **declared, never inferred**: a run that fell back to synthetic data cannot be detected from its outputs afterwards. `Verify` re-checks a manifest against disk | [examples/canonical/19-builtin-provenance-facets.ffl](examples/canonical/19-builtin-provenance-facets.ffl) |
...
- [gridbuilder](https://github.com/rlemke/fwh_gridbuilder) — OSM power infrastructure (substations, transmission lines) → model-ready grid CSV/GeoJSON per country, for PyPSA-Eur/PyPSA-Earth. The **FFL equivalent of [PyPSA grid-builder](https://github.com/PyPSA/grid-builder)** (Snakemake), calling the same `earth_osm` extractor so the comparison is between orchestrators, not payloads — verified same-data against it (832 rows, 0 differing). Ports only the *retrieve* stage, which is all upstream implements. **Fleet-deployed** (`service: runner-gridbuilder`, `task_list: grid`) — verified 2026-09-01 running on all four hosts. ⚠️ Each retrieval downloads a regional PBF, so `FW_GRIDBUILDER_DATA_DIR` points the earth-osm cache at the mounted data volume: a container-local cache re-downloads every restart and grows the Docker VM disk, which is how a concurrent fan-out previously took Mongo down with it
- [county-atlas](https://github.com/rlemke/fwh_county_atlas) — a map per US county over one catalog-driven generic renderer (`layers.json` is the whole integration surface); the 3,167-county national fan-out completed 0-failed. Fleet-deployed (`runner-county-atlas`). Design + as-built: [county-atlas.md](docs/architecture/county-atlas.md)
- [livability](https://github.com/rlemke/fwh_livability) — equal-weight US county livability composite (ACS + CDC PLACES + FEMA NRI + EPA AQI) → choropleth + top-50 rankings. ⚠️ Census GEOIDs here are AFF-style (`0500000US…`), so a bare-FIPS join silently yields an empty result rather than an error
- [energy](https://github.com/rlemke/fwh_energy) — US energy trade + prices (EIA) → dashboard. Reads EIA's **bulk NDJSON zips** rather than the API, so it needs no API key
- [uspanel](https://github.com/rlemke/fwh_uspanel) — US state × year multi-source data panel (Census ACS/PEP + BLS); 1,020 rows, 2005–2024. Needs `CENSUS_API_KEY`; PEP migration components exist only for 2010–2019
- [gibraltar](https://github.com/rlemke/fwh_gibraltar) — *The Hollow Mountain*: measuring what OpenStreetMap **cannot** see. Gibraltar's and Jersey's tunnel networks, where OSM holds ~12% of 55 km of known galleries. ⚠️ Where no total is published it returns `None`, never `0.0` — an unmeasured quantity is not a measured zero
- [peloton](https://github.com/rlemke/fwh_peloton) — group cycling photos → per-rider enhanced portraits (detect → crop → enhance; 16-bit TIFF, NEF→TIF)
- [groupphoto](https://github.com/rlemke/fwh_groupphoto) — posed group photos: deblur + glare/highlight correction + optional background replacement, one output per input

  ⚠️ `peloton` and `groupphoto` are **catalogued but deliberately skipped at image bake** (CV domains, heavy deps) — they run from a local checkout, not the fleet image. `energy`, `gibraltar` and `uspanel` are baked but carry **no `service`**, so they have no dedicated runner; their work is claimed by the generalist runner.

- [stocks](https://github.com/rlemke/fwh_stocks) — **paper-trading** stock-group tracker (not a map): factor screen + analyst consensus + optional LLM re-rank picks a group, entry prices are recorded, daily snapshots mark it to market, and close writes per-position and overall P/L **vs a benchmark**. Dashboard app at `/stocks`; pluggable `PriceProvider` (yfinance default, `stub` for offline tests); the full ranked candidate list is retained so unpicked names stay comparable
