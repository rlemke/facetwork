...
| **Built-in `fw.compare` facets** — `Tabular` / `Summarise`, for "did this pipeline reproduce that published result?". Reports the **highest level at which two outputs agree** (exists → schema → cardinality → keys → values → bytes) rather than a boolean, because a checksum is the wrong predicate: measured on `fwh_gridbuilder`, two orchestrators produced byte-different files containing identical data (unstable nested key order upstream). ⚠️ Normalisations are **declared and reported** in `normalised_by` — silently sorting until two files match manufactures a reproduction rather than demonstrating one. A missing record does not hide the rest: with `key_columns` the intersection is characterised too ("13,978 identical, 1 missing"). `Summarise` reports the **weakest** pair, and an empty verdict list is **not** success. ⚠️ **`out_of_scope` separates scope from content** — unmatched records lying in a key partition (first key column) the other artifact does not cover **at all**. Measured on GIAB HG001: of 242,958 records the ladder called "retired", **99,980 (41.2%) are chrX**, which the newer release simply does not include (`CHROM1-X` vs `1_22`). Reporting content differences between artifacts spanning **different key space** is the ladder's own failure mode one level up, and it runs green. `split_columns` explodes a **delimited multi-value column** into one row per element (a VCF `ALT` of `A,ATTTACAACC` is two alleles sharing a row) — it CHANGES the unit of the counts, so it says so in `normalised_by` with before/after | [examples/canonical/18-builtin-compare-facets.ffl](examples/canonical/18-builtin-compare-facets.ffl) |
