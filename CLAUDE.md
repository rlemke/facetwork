...
| Emergency-Access Atlas (`osm.emergency`, design): 4-level fan-out/fan-in showcase (continent→regions→cities→categories→routes); secondary-grade networks for city metrics, PBF-only facilities, bounded region tier, disclosed readiness weights | [docs/architecture/emergency-access-atlas.md](docs/architecture/emergency-access-atlas.md) |
