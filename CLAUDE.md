...
| **Built-in `fw.compare` facets** — `Tabular` / `Summarise`, for "did this pipeline reproduce that published result?". Reports the **highest level at which two outputs agree** (exists → schema → cardinality → keys → values → bytes) rather than a boolean, because a checksum is the wrong predicate: measured on `fwh_gridbuilder`, two orchestrators produced byte-different files containing identical data (unstable nested key order upstream). ⚠️ Normalisations are **declared and reported** in `normalised_by` — silently sorting until two files match manufactures a reproduction rather than demonstrating one. A missing record does not hide the rest: with `key_columns` the intersection is characterised too ("13,978 identical, 1 missing"). `Summarise` reports the **weakest** pair, and an empty verdict list is **not** success | [examples/canonical/18-builtin-compare-facets.ffl](examples/canonical/18-builtin-compare-facets.ffl) |
